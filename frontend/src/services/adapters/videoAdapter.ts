...
const callVeoApi = async (
  ...
  const aspectRatio = options.aspectRatio || model.params?.defaultAspectRatio || '16:9';
...
const callSoraApi = async (
  ...
  const aspectRatio = options.aspectRatio || model.params?.defaultAspectRatio || '16:9';
  const duration = options.duration || model.params?.defaultDuration || '15s';
  ...
  const { width, height, size } = getSizeFromAspectRatio(aspectRatio || '16:9');
...
export const callVideoApi = async (
  ...
  const provider = activeModel.providerId ? getProviderById(activeModel.providerId) : undefined;
  ...
  if (
    ...
    return generateDashScopeVideo({
      ...
      modelId: apiModel,
      apiKey,
      aspectRatio: options.aspectRatio || activeModel.params?.defaultAspectRatio || '16:9',
      duration: options.duration || activeModel.params?.defaultDuration || '15s',
    });
  }
...
export const isAspectRatioSupported = (
  ...
  return activeModel.params?.supportedAspectRatios?.includes(aspectRatio) || false;
};
...
export const isDurationSupported = (
  ...
  return activeModel.params?.supportedDurations?.includes(duration) || false;
};
