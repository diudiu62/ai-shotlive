...
import { Key, CheckCircle, ExternalLink, Gift, Sparkles, Eye, EyeOff, Save, Trash2, Plus, Globe, Edit3, Loader2 } from 'lucide-react';
import { 
  getProviders, 
  updateProvider,
  addProvider,
...
const GlobalSettings: React.FC<GlobalSettingsProps> = ({ onRefresh }) => {
  ...
  const handleAddProvider = (name: string, baseUrl: string, apiKey?: string) => {
    addProvider({ name, baseUrl, apiKey: apiKey || '', isDefault: false, isEnabled: true });
    setIsAdding(false);
    loadProviders();
    onRefresh();
  };
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-[var(--text-tertiary)] mb-3 leading-relaxed">
              ...
              稳定快速，价格优惠。本开源项目由 AntSK API 提供支持。
            </p>
