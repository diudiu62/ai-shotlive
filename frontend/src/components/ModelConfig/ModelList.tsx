...
const ModelList: React.FC<ModelListProps> = ({ type, onRefresh }) => {
  ...
  const loadModels = () => {
    ...
    setActiveModelId((activeConfig as any)[type] || '');
  };
  ...
  const handleSetActiveModel = (modelId: string) => {
    ...
    const provider = model && model.providerId ? getProviderById(model.providerId) : null;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[var(--accent-bg)] border border-[var(--accent-border)] rounded-lg p-3">
        ...
        {(() => {
          ...
          const provider = activeModel && activeModel.providerId ? getProviderById(activeModel.providerId) : null;
          return (
