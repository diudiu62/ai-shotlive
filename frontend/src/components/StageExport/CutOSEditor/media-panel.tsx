...
function MediaTab({ mediaFiles, onFilesAdded, onRemoveFile, projectId, onReindexMedia }: MediaTabProps) {
  ...
  const performNlpSearch = useCallback(async (query: string) => {
    ...
    const indexedMedia = mediaFiles.filter(m => (m as any).twelveLabsStatus === "ready")
    ...
    try {
      const response = await fetch("/api/twelvelabs/search", {
        ...
        body: JSON.stringify({
          projectId,
          query: query.trim(),
          videoIds: indexedMedia.map(m => (m as any).twelveLabsVideoId).filter(Boolean),
        }),
      })
      ...
      const mappedResults: NLPSearchResult[] = data.results.map((r: { videoId: string; start: number; end: number; rank: number }) => {
        const media = mediaFiles.find(m => (m as any).twelveLabsVideoId === r.videoId)
        return {
          ...r,
          mediaId: media?.id,
          media,
  ...
  const hasIndexedMedia = nlpAvailable && mediaFiles.some(m => (m as any).twelveLabsStatus === "ready")
  const indexingCount = nlpAvailable ? mediaFiles.filter(m => (m as any).twelveLabsStatus === "indexing" || (m as any).twelveLabsStatus === "pending").length : 0
              ...
              {filteredFiles.map((media, index) => (
                ...
                >
                  ...
                  <div className="absolute top-1.5 left-1.5 flex gap-1">
                    ...
                    {(media as any).twelveLabsStatus === "indexing" || (media as any).twelveLabsStatus === "pending" ? (
                      <motion.div
                    ...
                    ) : (media as any).twelveLabsStatus === "ready" ? (
                      <motion.div
                        className="rounded-full bg-cyan-500/80 p-1"
                        title="Searchable"
                        initial={{ scale: 0 }}
                    ...
                    ) : (media as any).twelveLabsStatus === "failed" ? (
                      <motion.div
                        className="rounded-full bg-red-500/80 p-1"
                        title={(media as any).twelveLabsError ? `Failed: ${(media as any).twelveLabsError}` : "Indexing failed"}
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ type: "spring", stiffness: 500, damping: 25 }}
                      >
                  ...
                  <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {media.storageUrl && (!(media as any).twelveLabsStatus || (media as any).twelveLabsStatus === "failed") && (
                      <motion.button
                        onClick={(e) => {
                          e.stopPropagation()
                        ...
                        title={(media as any).twelveLabsStatus === "failed" ? "Retry indexing" : "Make searchable"}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                      >
