...
import { ProjectState, CharacterVariation, Character, Scene, Prop, AspectRatio, AssetLibraryItem, CharacterTurnaroundPanel } from '@/app/types/types';
import { generateVisualPrompts, generateCharacterTurnaroundPanels, generateCharacterTurnaroundImage, getStylePrompt, CHARACTER_TURNAROUND_LAYOUT } from '../../services/aiService';
...
import { createLibraryItemFromCharacter, createLibraryItemFromScene, createLibraryItemFromProp, cloneCharacterForProject, cloneSceneForProject, clonePropForProject } from '../../services/assetLibraryService';
    ...
    try {
      ...
      if (item.type === 'character') {
        ...
        PS.addCharacter(project.id, {
          ...
          variations: (character.variations || []).map((v: CharacterVariation) => ({ ...v, referenceImage: stripAuthToken(v.referenceImage) })),
        });
...
⚠️ CHARACTER CONSISTENCY IS THE #1 PRIORITY - The character must look like the EXACT SAME PERSON in all 9 panels!`;
  ...
  const selectedChar = selectedCharId ? project.scriptData.characters.find(c => compareIds(c.id, selectedCharId)) : undefined;
          ...
          <div className="flex items-center gap-2">
            ...
            <AspectRatioSelector
              ...
              allowSquare={(() => {
                ...
                return (activeModel?.params as any)?.supportedAspectRatios?.includes('1:1') ?? false;
              })()}
              disabled={!!batchProgress}
            />
          </div>
