...
import { NovelChapter, NovelEpisode } from '@/app/types/types';
    ...
    if (onDelta) {
      ...
      result = await retryOperation(() =>
        chatCompletionStream(
          ...
          return chatCompletion(fullPrompt, model, 0.7);
        })
      );
    } else {
      result = await retryOperation(() =>
        chatCompletion(fullPrompt, model, 0.7)
      );
    }
    ...
    if (onDelta) {
      ...
      result = await retryOperation(() =>
        chatCompletionStream(
          ...
          logScriptProgress('流式输出失败，回退到普通模式...');
          return chatCompletion(fullPrompt, model, 0.8);
        })
      );
    } else {
      result = await retryOperation(() =>
        chatCompletion(fullPrompt, model, 0.8)
      );
    }
