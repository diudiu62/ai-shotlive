...
import { AspectRatio, NineGridPanel } from "@/app/types/types";
...
`;
  ...
  try {
    const result = await retryOperation(() => chatCompletion(prompt, model, 0.7, 'json_object'));
...
注意：必须恰好返回9个panel（index 0-8），按照九宫格从左到右、从上到下的顺序排列。`;
  ...
  try {
    const responseText = await retryOperation(() => chatCompletion(fullPrompt, resolvedModel, 0.7, 'json_object'));
    const duration = Date.now() - startTime;
