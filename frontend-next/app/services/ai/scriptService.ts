...
import { ScriptData, Shot, Scene, ArtDirection } from "@/app/types/types";
...
export const parseScriptToData = async (
  ...
  try {
    const responseText = await retryOperation(() => chatCompletion(prompt, model, 0.7, 'json_object'));
    ...
    try {
      ...
      console.log("✅ 全局美术指导文档生成完成，风格关键词:", artDirection?.moodKeywords?.join(', ') || '无');
...
${artDirectionBlock}
    ...
    try {
      ...
      responseText = await retryOperation(() => chatCompletion(prompt, model, 0.5, 'json_object'));
      const text = cleanJsonString(responseText);
...
`;
  ...
  try {
    const result = await retryOperation(() => chatCompletion(prompt, model, 0.8));
...
`;
  ...
  try {
    const result = await retryOperation(() => chatCompletion(prompt, model, 0.7));
    const duration = Date.now() - startTime;
...
import { QualityCheck, ShotQualityAssessment } from "@/app/types/types";
...
const evaluateAssetCoverage = (shot: Shot, scriptData?: ScriptData | null): QualityCheck => {
  ...
  const characterScore = charScoreParts.length
    ? charScoreParts.reduce((sum, value) => sum + value, 0 as number) / charScoreParts.length
    : 20;
  ...
  const propScore = propScoreParts.length
    ? propScoreParts.reduce((sum, value) => sum + value, 0 as number) / propScoreParts.length
    : 10;
...
`;
    ...
    try {
      const result = await retryOperation(() => chatCompletion(prompt, model, 0.7, 'json_object'), 2, 2000);
