...
import { Character, Scene, Prop, AspectRatio, ArtDirection, CharacterTurnaroundPanel } from "@/app/types/types";
import { ImageApiFormat, ImageModelDefinition } from "@/app/types/model";
...
Output ONLY valid JSON with this exact structure:
{
  ...
  try {
    const responseText = await retryOperation(() => chatCompletion(prompt, model, 0.4, 'json_object'));
...
Output ONLY the visual prompt text, no explanations.`;
  ...
  const visualPrompt = await retryOperation(() => chatCompletion(prompt, model, 0.5));
