...
// localStorage 键名
const STORAGE_KEY = "AiDrama_model_registry";
...
const getDefaultState = (): ModelRegistryState => ({
  providers: [...BUILTIN_PROVIDERS.map(p => ({ ...p, apiKey: '', isEnabled: true, isBuiltIn: true }))],
  models: [...ALL_BUILTIN_MODELS.map(m => ({ ...m, provider: m.providerId, type: (m.type === 'text' ? 'chat' : m.type) as any })) as any],
  ...
  try {
    ...
    if (stored) {
      ...
      BUILTIN_PROVIDERS.forEach(bp => {
        ...
        if (existingIndex === -1) {
          parsed.providers.unshift({ ...bp, apiKey: '', isEnabled: true, isBuiltIn: true });
        } else {
      ...
      if (removedProviderMap.size > 0) {
        parsed.models.forEach(m => {
          const newProviderId = m.providerId ? removedProviderMap.get(m.providerId) : undefined;
          if (newProviderId) {
      ...
      ALL_BUILTIN_MODELS.forEach(bm => {
        ...
        if (existingIndex === -1) {
          // 内置模型不存在，添加
          parsed.models.push({ ...bm, provider: bm.providerId, type: (bm.type === 'text' ? 'chat' : bm.type) as any });
        } else {
          ...
          parsed.models[existingIndex] = {
            ...
            provider: existing.provider || bm.providerId,
            type: (bm.type === 'text' ? 'chat' : bm.type) as any,
...
export const getDefaultProvider = (): ModelProvider => {
  return getProviders().find(p => p.isDefault) || { ...BUILTIN_PROVIDERS[0], apiKey: '', isEnabled: true, isBuiltIn: true };
};
...
export const getActiveModel = (type: ModelType): ModelDefinition | undefined => {
  const state = loadRegistry();
  const activeId = state.activeModels[type as keyof typeof state.activeModels];
  return activeId ? getModelById(activeId) : undefined;
};
...
export const setActiveModel = (type: ModelType, modelId: string): boolean => {
  ...
  const provider = model.providerId ? getProviderById(model.providerId) : undefined;
...
export const getAvailableModels = (type?: ModelType): ModelDefinition[] => {
  const state = loadRegistry();
  const providerMap = new Map<string, ModelProvider>(state.providers.map(p => [p.id, p]));
  const models = type ? state.models.filter(m => m.type === type) : state.models;
  return models.filter(m => m.isEnabled && m.providerId && !!providerMap.get(m.providerId)?.apiKey);
};
...
export const getDefaultAspectRatio = (): AspectRatio => {
  ...
  if (imageModel && imageModel.params?.defaultAspectRatio) {
    return imageModel.params.defaultAspectRatio as AspectRatio;
  }
  return '16:9';
};
...
export const getDefaultVideoDuration = (): VideoDuration => {
  ...
  if (videoModel && videoModel.params?.defaultDuration) {
    return videoModel.params.defaultDuration as VideoDuration;
  }
  return '10s';
};
