...
const callGeminiImageApi = async (
  ...
  const aspectRatio = options.aspectRatio || activeModel.params?.defaultAspectRatio || '16:9';
...
const callDashScopeImageApi = async (
  ...
  const requestBody: Record<string, any> = {
    ...
    parameters: {
      size: mapAspectRatioToDashScopeSize(aspectRatio as AspectRatio),
      n: 1,
      prompt_extend: true,
      watermark: false,
    },
...
export const callImageApi = async (
  ...
  const apiFormat: ImageApiFormat = (activeModel.params as any)?.apiFormat || 'gemini';
...
export const isAspectRatioSupported = (
  ...
  return (activeModel.params as any)?.supportedAspectRatios?.includes(aspectRatio) || false;
};
